...
def get_content_loss(x, c):
    with tf.name_scope('get_content_loss'):
        # Get the activated VGG feature maps and compute the mean-squared error
        content_noise_out = getattr(x, content_layer)
        content_photo_out = getattr(c, content_layer)
        squared_errors = tf.square(content_noise_out - content_photo_out)
        return tf.reduce_sum(squared_errors) / 2.
...
def get_style_loss(x, s):
    with tf.name_scope('get_style_loss'):
        ...
        style_weights = tf.constant([.2] * len(style_layer_losses), tf.float32)
...
# Compute the (p) norm of a given image (x)
def get_norm(x, p):
    summed_terms = tf.reduce_sum(tf.pow(x, p))
    return tf.pow(summed_terms, 1. / p)
...
def get_style_loss_for_layer(x, s, l):
    with tf.name_scope('get_style_loss_for_layer'):
        ...
        with tf.control_dependencies([assert_equal_shapes]):
            ...
            style_fraction = 1. / (4 * style_layer_elements)
...
with tf.Session() as sess:
    ...
    with tf.name_scope('loss'):
        ...
        if norm_weight is 0:
            ...
            norm_loss = get_norm(noise, 2.) * norm_weight
        ...
        # Total loss
        total_loss = tf.sqrt(content_loss + style_loss + norm_loss + tv_loss)
    ...
    for i in range(epochs):
        if i % 200 == 0:
            print("Epoch %04d | Total Error is %.3f" % (i, sess.run(total_loss)))
